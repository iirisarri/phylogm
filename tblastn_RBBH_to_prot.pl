...
my $usage = "tblastn_RBBH_to_prot.pl blast_report_1 blast_report_2 source_fasta > output.fa\n";
...
print STDERR "extracting RBBH from blast report #1 and translating into protein...\n";
...
while (my $seq_obj = $seqio_obj->next_seq){
	...
	if ( $seqname eq $blast_1{$key}{'blast_info'}[0] ) {
	    ...
	    foreach my $fram ( @{ $blast_1{$key}{'frames'} } ) {
