...
# modification of gene_jackknife_from_fasta_collection.pl to print out partitions
#
...
for (my $i=0; $i<$num_replicates; $i++) {
    ...
    my @replicate = ();
    my %partitions;
	my $order = 0; # traces order of genes in replicate
	...
	while ( $total_length < $replicate_size ) {
		...
		while (my $inseq = $seqio_obj->next_seq) {
			...
			if ( !exists $partitions{$order} ) {
				...
				$partitions{$order} = [$gene, $new_seq_length];
			}
		}
	...
	my $outpart = $outfile_pattern . "_rep${i}.part";
	...
	foreach my $part ( sort keys %partitions ) {
		...
		# for all other alignments
			...
			my $start = $prev_end + 1;
			my $end = $prev_end + $len;
			print PART "$datatype, $aln = $start - $end\n";
